\documentclass[compress,red,handout,10pt]{beamer}
\usepackage{etex}
\mode<presentation>
%\usetheme{Warsaw}
...
%\usepackage{epsfig}
\usepackage{graphicx}
\graphicspath{{assets/images/}{assets/img/}}
...
\institute[VTAK]{Tallinna Tehnika\"ulikool \\
  %\texttt{taavi.pall@ttu.ee}
}
\date[oktoober 2015]{22. oktoober 2015}
...
\begin{center}
\begin{figure}
\includegraphics[width=.6\textwidth]{Zetterberg}
...
\includegraphics[width=\textwidth]{Hartwell1973}
\end{center}
\end{column}
\begin{column}{.5\textwidth}
\begin{block}{}
\begin{itemize}
 \item Leland H. (Lee) Hartwell jt. kirjeldasid 1970ndate alguses terve rea \emph{Saccharomyces cerevisiae} jagunemise mutante
...
% \frame{\frametitle{MPF merisiilikus}
...
\begin{column}{.5\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{Hunt1982.jpg}
\end{center}
\end{column}
\begin{column}{.5\textwidth}
\begin{block}{}
\begin{itemize}
 \item Nad nimetasid need ja teised sarnaselt k\"aituvad rakuts\"uklis ostsilleeruvad valgud \textbf{ts\"ukliinideks}
\item K\~{o}igepealt kirjeldati kaks ts\"ukliini - A ja B
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Ts\"ukliini valgu s\"untees on vajalik rakuts\"ukli t\"o\"os hoidmiseks}
...
\end{block}
\begin{center}
\includegraphics[width=0.8\textwidth]{MurrayandKirchner1989}
\end{center}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
